...
docker build -t flask-library:latest app
...
#run nginx container
docker run -d --name nginx -p 80:80 --network mynetwork --mount type=bind,source=$(pwd)/nginx/nginx.conf,target=/etc/nginx/nginx.conf nginx:latest
